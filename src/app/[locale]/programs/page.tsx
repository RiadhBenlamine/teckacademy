      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-20">
          {learningPaths.map((path) => (
            <Card
              key={path.id}
              className="flex flex-col justify-between border-slate-200/90 dark:border-slate-800 group hover:border-blue-400/40 dark:hover:border-cyan-500/40 p-6"
            >
              <div>
                ...
                <p className="mt-2.5 text-sm text-slate-600 dark:text-slate-300 leading-relaxed min-h-[50px]">
                ...
                <div className="mt-5 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {path.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-medium border border-slate-200/60 dark:border-slate-700"
                      >
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                <Link
                  href={`/${locale}/contact`}
                  className="w-full inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-blue-50 dark:bg-slate-800 hover:bg-blue-600 dark:hover:bg-blue-600 text-blue-700 dark:text-cyan-400 hover:text-white dark:hover:text-white font-semibold text-sm transition-all"
                >
        ...
        <div className="pt-8 border-t border-slate-200/70 dark:border-slate-800">
          <SectionHeading
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {ageGroupsData.map((group) => (
              <Card key={group.id} className="p-6 border-slate-200/90 dark:border-slate-800 relative overflow-hidden">
                <div className={`absolute top-0 inset-x-0 h-1.5 ${group.indicatorColor}`} />
                <div className="flex items-center justify-between mb-3">
                  <span className="font-extrabold text-lg text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-200/80 dark:border-slate-700">
